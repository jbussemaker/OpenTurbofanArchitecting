...
def _get_problem(fuel_problem, fuel_choice):
    return ArchitectingProblem(
        analysis_problem=fuel_problem, choices=[fuel_choice],
        objectives=[TSFCMetric()], constraints=[TSFCMetric()], metrics=[TSFCMetric()],
    )


def test_des_vars(fuel_problem):
    problem = _get_problem(fuel_problem, FuelChoice())
...
def test_modify_architecture(fuel_problem):
    problem = _get_problem(fuel_problem, FuelChoice())
